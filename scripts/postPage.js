...
async function postPage(provision, index, finalResolve) {
...
//    text = text + " }}"
    ...
    console.debug("posting page", title, "properties", provision);

    if (title.indexOf("undefined") === -1) doEditRequest(title,text, finalResolve);
    else { finalResolve();
    }
    resolve()
    })
...
async function doEditRequest(title,text, finalResolve) {
    await editRequest(title,text, finalResolve);
}
...
await postPage('test', 0, () => {console.debug("done")});
...
async function editRequest(title,text, finalResolve) {
    return new Promise(resolve => {
        var params_3 = {
            action: "edit",
            title,
        ...
        console.debug("posting!",title,text)
        ...
        // api.php?action=delete&title=title&token=
         request.get({ url: url, form: params_5 }, function (error, res, body) {
                            console.log(body);
                            if (error) {
                                console.log(error);
